...
use ron::de;
use std::{sync::Arc, time::Instant, arch::x86_64::_mm_movedup_pd};
use vulkano::{
    buffer::{
    ...
    format::Format,
    image::{view::ImageView, AttachmentImage, ImageAccess, ImageUsage, SwapchainImage, swapchain},
...
pub const NORMALS: [Normal; 8] = [
	 Normal { normal: [-0.0000, 1.0000, -0.0000] },
	 Normal { normal: [-0.0000, -0.0000, 1.0000] },
	 Normal { normal: [-1.0000, -0.0000, -0.0000] },
	 Normal { normal: [-0.0000, -1.0000, -0.0000] },
	 Normal { normal: [1.0000, -0.0000, -0.0000] },
	 Normal { normal: [-0.0000, -0.0000, -1.0000] },
	 Normal { normal: [1.0000, -0.0000, -0.0000] },
...
fn main() 
{
	...
	let descritor_set_allocator = StandardDescriptorSetAllocator::new(device.clone());
